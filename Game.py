...
class GameTile:
			...
			if clickPos[0] > self.xPos and clickPos[0] < self.xPos + self.width:
				if clickPos[1] > self.yPos and clickPos[1] < self.yPos + self.height:
					...
					if self.ship != None:
						...
						self.ship.hit()
						self.blowup_animation(clickPos)
						self.shipDestroyedAnimation()
					else:
						self.setStatus(self.MISS)
						self.splash_animation(clickPos)
						#Miss Sound
	...
	def blowup_animation(self,clickPos):
		for image in Explosion_Images:
			image = pg.transform.scale(image, (20,20))
			self.surface.blit(image, clickPos)
			pg.display.flip()
			clock.tick(ExplosionSpeed)

	def splash_animation(self,clickPos):
